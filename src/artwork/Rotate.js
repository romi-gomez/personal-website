    ...
    Sketch = (p) => {
        ...
        p.draw = () => {                  
            ...
            //* START GRID CODE------------------------------------------------------------------*/
                ...
                for (let x=0; x<p.width; x+=p.width/columns){
                    for (let y=0; y<p.height; y+=p.height/rows){
                        p.noFill()
                        p.stroke(255,255, 255, p.random(3))
                        p.circle(x-p.width/2, y-p.height/2, amplitude*50)
                    }
                }
