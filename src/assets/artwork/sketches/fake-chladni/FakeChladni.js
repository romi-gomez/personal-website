...
class FakeChladni extends React.Component {
    constructor() {
        super()
    ...
    Sketch = (p) => {
        ...
        p.draw = () => {                  
            ...
            //* START GRID CODE------------------------------------------------------------------*/
                p.fill(backgroundColor,p.random(5))
                ...
                for (let x=p.width/columns+p.width/columns/2; x<p.width+100; x+=p.width/columns){
                  // p.rotate(30+p.int(amplitude*2))
                    for (let y=p.height/rows+p.height/rows/2; y<p.height+100; y+=p.height/rows){
                        ...
                        p.circle(xcoord-(p.width/columns)/2, ycoord-(p.height/rows)/2, amplitude*30)
